...
class PlotApp(QWidget):
    ...
    def initUI(self):
        ...
        pixmap = QPixmap('C:\\Users\\Adrija\\Desktop\\dnvproject\\DNV-logo.jpg')
    ...
    def generate_plot(self):
        ...
        try:
            ...
            footnotes = []
            if a_col:
                footnotes.append(f'Y1 Max: {data_a.max()}, Min: {data_a.min()}')
            if b_col:
                footnotes.append(f'Y2 Max: {data_b.max()}, Min: {data_b.min()}')
            if c_col:
                footnotes.append(f'Y3 Max: {data_c.max()}, Min: {data_c.min()}')
